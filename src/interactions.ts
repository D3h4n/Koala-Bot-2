...
function echoCommand(options: Record<string, string | undefined>, replier: MessageReplier) {
...
function chooseCommand(options: Record<string, string | undefined>, replier: MessageReplier) {
  const choices = Object
      .entries(options)
      .filter(([key, value]) => key.match(/choice[1-9]/) && typeof value === 'string')
      .map((entry) => <string>entry[1])

  assert.ok(choices.length >= 2, "ERROR: There should always be at least 2 choices")
